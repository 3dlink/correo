...
class User extends UserMgmtAppModel {
	 ...
	 * @var array
	 */
	var $belongsTo = array('Usermgmt.UserGroup', 'Entity', 'Group');
	/**
	...
	function LoginValidate() {
		$validate1 = array(
				'email'=> array(
					'mustNotEmpty'=>array(
						'rule' => 'notEmpty',
						'message'=> 'Ingrese el correo y nombre de usuario')
					),
				'password'=>array(
					'mustNotEmpty'=>array(
						'rule' => 'notEmpty',
		...
		$validate1 = array(
				...
				'username'=> array(
					...
					'mustBeLonger'=>array(
						...
						'message'=> 'El nombre de usuario debe contener al menos cuatro caracteres',
						'last'=>true),
					),
				'first_name'=> array(
					'mustNotEmpty'=>array(
				...
				'email'=> array(
					'mustNotEmpty'=>array(
						'rule' => 'notEmpty',
						'message'=> 'Ingrese la dirección de coreo electrónico',
						'last'=>true),
					'mustBeEmail'=> array(
						'rule' => array('email'),
				...
				'oldpassword'=>array(
					'mustNotEmpty'=>array(
						'rule' => 'notEmpty',
						'message'=> 'Ingrese la contraseña anterior',
						'last'=>true),
					'mustMatch'=>array(
				...
				'password'=>array(
					'mustNotEmpty'=>array(
						'rule' => 'expre',
						'message'=> 'La contraseña debe tener al menos una letra mayúscula, una letra minúscula, un número o carácter especial (- # & @ ! ? _ .), como mínimo 8 caracteres',
						'on' => 'create',
						'last'=>true),
					// 'mustBeLonger'=>array(
	...
	public function expre() {	
		return ( !(7>= strlen($_POST['data']['User']['password'])) && ereg("([A-Z])",$_POST['data']['User']['password']) && ereg("([a-z])",$_POST['data']['User']['password']) && ereg("([0-9])",$_POST['data']['User']['password']) && ereg("([-#&@!?_.])", $_POST['data']['User']['password']));
	}
	/**
	 ...
	 * @return void
	 */
	public function sendMailActive($user, $userAdmin) {
		...
		$email->to($userAdmin['User']['email']);
		$email->subject('Activar usuario');
		//$email->transport('Debug');
		$body="El usuario ".$user['User']['first_name']." ".$user['User']['last_name'].", ha sido desactivado por cumplir con sus 3 intentos de logueo.";
		try{
			$result = $email->send($body);
		} catch (Exception $ex) {
			// we could not send the email, ignore it
			$result="sendMailActive Could not send registration email to userid-".$userId;
		}
	...
	public function forgotPassword($user) {
		...
		$userId=$user['User']['id'];
		$email = new CakeEmail('_3tp');
		$fromConfig = EMAIL_FROM_ADDRESS;
		$fromNameConfig = EMAIL_FROM_NAME;
		$email->from(array( $fromConfig => $fromNameConfig));
		$email->sender(array( $fromConfig => $fromNameConfig));
		$email->to($user['User']['email']);
		$email->subject(EMAIL_FROM_NAME.': Solicitud de reestablecimiento de contraseña');
		$activate_key = $this->getActivationKey($user['User']['password']);
		$link = Router::url("/activatePassword?ident=$userId&activate=$activate_key",true);
		$body= "Hola ".$user['User']['first_name'].", has solicitado restablecer tu contraseña para ingresar al Sistema de Correspondencia Estatal. Si es así, haz click en el link que se muestra a continuación:
		".$link." Si el link mostrado no funciona, copia y pega el link en la barra del buscador para poder restablecer tu contraseña.

		Escoje una contraseña que puedas recordar y mantenla asegurada.
...
EMAIL_FROM_NAME;
	...
	// obtener los usuarios de una entidad
	public function getUsersFromEntity($idEntity){
		$users = $this->find('all', array(
			'conditions' => array(
				'User.entity_id' => $idEntity,
				'visible' => 1
				),
	...
	public function findUsersByName($q){
		$this->recursive = -1;
		$users = $this->find('all', array(
			'conditions' => array(
				'OR' => array(
	...
	// buscar usuarios con nombres parecidos
	public function findIdUsersByName($q){
		$this->recursive = -1;
