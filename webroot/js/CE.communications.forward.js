$.extend(CE.communications, {
	...
	forward: function() {
		...
		$('#btn-send-message').on('click', function(event) {
			...
			// agregar los documentos del original al nuevo
	        var data = {
	        	message_id: $('#btn-send-message').attr('data-message-id'),
	        	temporal_id: $('#txtIdMessage').val()
	        }

	       	/*$.ajax({
	      		url: WEBROOT + 'communications/addPreUpload/',
	      		data: data,
	      		type: 'post',
	      		success: function(data){
			...
			var history = $('.HistoryMessage').length;
	    	var msgHistory = [];
	    	for (var i = 1; i <= history; i++) {
	    		msgHistory[i] = $('#M'+i).val();
	    	};
	    	msgHistory.shift();
		...
		$(document).on('click','#btn-find-by-group', function(e) {
			...
			// btn para buscar a todos los usuarios que estan en el grupo
		$(document).on('click','#btn-find-by-groupi', function() {
			event.preventDefault();
		...
		// btn para buscar a todos los usuarios que estan en el grupo
		$(document).on('click','#btn-find-by-circle', function() {
			event.preventDefault();
...
$('#btn-send-message').on('click', function(event) {
    ...
    $('.btn-file-sign1').on('click', function(){
